...
def sort_scram_words(cleaned_scrammbled_word_list):
    sortedTestWords = []
    for word in xrange(len(cleaned_scrammbled_word_list)):
        tempString = ""
...
def wordlist_dictionary_creator(cleaned_word_list):
    sortedTestWords = []
    for word in xrange(len(cleaned_word_list)):
        tempString = ""
...
def read_wordlist(word_list):
    ...
    return wordlist_dictionary_creator(clean_word_list) #pass wordlist to wordlist_dictionary_creator() and return the dictionary
        


#Log in and open html page for reading 
hts = HackThisSite(XXXXXXXXX, XXXXXXXX) # constructor takes username and password for hts
...
#create a dictionary where the keys are the words from the wordlist and the value is the same word sorted alphanumerically
...
dirty_list = str(html_page)[start_of_list+42:end_of_list-282]  #by adding 42 places and subtracting 282 places the html content is clearer
...
cleaned_scrambled_list = filter(None, cleaned_scrambled_list) #makes sure there are no empty items in list, we a list length of ten

sorted_scrammbled_list = sort_scram_words(cleaned_scrambled_list)
 

string = ""
for word in sorted_scrammbled_list:
